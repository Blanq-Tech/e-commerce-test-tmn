...
### Офлайн-режим (детерминированная проверка)

Живой Ozon агрессивно фингерпринтит трафик и требует рабочих резидентных RU-прокси.
Чтобы можно было проверить логику и устойчивость без зависимости от антибота, есть
офлайн-режим — тот же код (браузер → разбор → оркестрация) гоняется по сохранённой
странице выдачи:

```bash
python main.py --query "нож туристический" --sku 1635725435 \
  --fixture data/sample_search.html
# -> position: 17, status: ok
```
...
По умолчанию `stability` запускается в офлайн-режиме (фикстура) и даёт
детерминированные 3/3. Для боевого прогона уберите `FIXTURE` из
`docker-compose.yml` и смонтируйте рабочий `proxies.txt`.
